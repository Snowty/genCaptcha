...
def get_img_text(angle=(-5,5),root='data/0',length=10):    #修改字符个数
    print("haha")
    try:
...
fonts      = glob.glob('./font/*.*')##字体集
crupsPaths = glob.glob('./text/*.txt')##语料库
maxLen =10##每行字符个数
if __name__ == '__main__':
     for i in range(3):
        get_img_text(angle=(0,0),root='./captcha_bgpic')
        #get_img_text(angle=(-0.5,0.5))
